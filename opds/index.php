...
header("Content-type: " . ($pageformat === 'opds' ? 'application/atom+xml' : 'text/html'));
...
function parse_titles(&$tpl,&$db,$offset,$all,$url,$prefix='titles',$searchvals='') {
    ...
    while ( $db->next_record() ) {
        ...
        $tpl->set_var('title',str_replace('{title}', $db->f('title'), str_replace('{author}', $db->f('name'), '{title} from {author}')));
